...
export function KatzenverwaltungDialog({ open, onClose, onSubmit, defaultValues, kundenverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: KatzenverwaltungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "rasse": string | null, // Rasse\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "geschlecht": LookupValue | null, // Geschlecht (select one key: "maennlich" | "weiblich" | "unbekannt") mapping: maennlich=Männlich, weiblich=Weiblich, unbekannt=Unbekannt\n  "farbe": string | null, // Fellfarbe\n  "impfstatus": LookupValue | null, // Impfstatus (select one key: "vollstaendig" | "teilweise" | "nicht_geimpft" | "unbekannt_impf") mapping: vollstaendig=Vollständig geimpft, teilweise=Teilweise geimpft, nicht_geimpft=Nicht geimpft, unbekannt_impf=Unbekannt\n  "besonderheiten": string | null, // Besonderheiten / Gesundheitshinweise\n  "besitzer": string | null, // Display name from Kundenverwaltung (see <available-records>)\n  "name": string | null, // Name der Katze\n}`;
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={lookupKey(fields.impfstatus) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, impfstatus: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="impfstatus"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="vollstaendig">Vollständig geimpft</SelectItem>
                <SelectItem value="teilweise">Teilweise geimpft</SelectItem>
                <SelectItem value="nicht_geimpft">Nicht geimpft</SelectItem>
                <SelectItem value="unbekannt_impf">Unbekannt</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="name">Name der Katze</Label>
            <Input
              id="name"
              value={fields.name ?? ''}
