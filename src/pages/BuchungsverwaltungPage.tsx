...
export default function BuchungsverwaltungPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, kundenverwaltungData, katzenverwaltungData, zusatzleistungenData] = await Promise.all([
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anreisedatum')}>
                <span className="inline-flex items-center gap-1">
                  Anreisedatum
                  {sortKey === 'anreisedatum' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('box_nummer')}>
                <span className="inline-flex items-center gap-1">
                  Box-/Zimmernummer
                  {sortKey === 'box_nummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zusatzleistungen_buchung')}>
                <span className="inline-flex items-center gap-1">
                  Zusatzleistungen
                  {sortKey === 'zusatzleistungen_buchung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notizen_buchung')}>
                <span className="inline-flex items-center gap-1">
                  Notizen zur Buchung
                  {sortKey === 'notizen_buchung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell className="text-muted-foreground">{formatDate(record.fields.anreisedatum)}</TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.abreisedatum)}</TableCell>
                <TableCell className="font-medium">{record.fields.box_nummer ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getZusatzleistungenDisplayName(record.fields.zusatzleistungen_buchung)}</span></TableCell>
                <TableCell>{record.fields.gesamtpreis ?? '—'}</TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.notizen_buchung ?? '—'}</span></TableCell>
                <TableCell>
